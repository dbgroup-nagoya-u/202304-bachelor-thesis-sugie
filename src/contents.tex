...
よって，UART（改善）は有用性が高い索引であることが分かった．

UART（改善）は読み書き両方において，その操作に特化した索引に近い性能を持つことが分かった．
...
例えば，UB木ではデータ層も索引層も構造変化させやすい．
索引層はキーの値しか保持しないため，構造変化の処理が単純なためである．
一方で，空間の情報を無視して多次元キーを１次元キーに変えてしてまうため，範囲検索時には断続的な検索が多くなり低性能となる．
